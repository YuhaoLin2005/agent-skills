...
def check_degradation_gate(root_missing, memory_missing):
    ...
    SEVERE → write .degraded-session flag + strong stderr signal
    MEDIUM → write .degraded-medium flag + stderr signal (v3.1: was prose-only)
    MINOR  → informational only
    Returns True if session should proceed, False if blocked.

    v3.1 (2026-07-11): HARDENED — MEDIUM now writes flag (was pure prose).
      Dual-pool audit: 6/7 degradation levels were prose → now MEDIUM produces
      mechanical evidence. SEVERE still writes flag (exit 2 would block recovery
      work — BODY.md says session CAN proceed degraded)."""
    all_missing = {**root_missing, **memory_missing}

    # FATAL gate: session cannot proceed (settings.json / INTERFACE.md / CLAUDE.md)
    fatals = [f for f, s in all_missing.items() if s == "FATAL"]
    if fatals:
        print(f"DEGRADE:GATE:FATAL:{','.join(fatals)}:session_blocked", file=sys.stderr)
        return False

    # SEVERE gate: write flag for Stop hooks + strong signal (BODY.md)
    ...
    if severes:
        ...
        print(f"DEGRADE:JSON:{json.dumps({'action': 'degraded_session', 'severity': 'SEVERE', 'files': severes, 'effect': 'dual-pool review + delivery gate skipped, output marked unverified'})}", file=sys.stderr)

    # MEDIUM gate: write flag (NEW in v3.1 — was pure prose before dual-pool audit)
    mediums = [f for f, s in all_missing.items() if s == "MEDIUM"]
    if mediums:
        flag_path = os.path.join(MEMORY, ".degraded-medium")
        with open(flag_path, "w") as f:
            f.write(f"degraded-medium:{','.join(mediums)}\n")
        print(f"DEGRADE:GATE:MEDIUM:{','.join(mediums)}:flag_written", file=sys.stderr)
        print(f"DEGRADE:JSON:{json.dumps({'action': 'degraded_medium', 'severity': 'MEDIUM', 'files': mediums, 'effect': 'dual-pool unavailable, fallback to 3x code-reviewer; identity may be degraded'})}", file=sys.stderr)

    # MINOR: informational only (self-model.md, assumption.md)
    for f, s in all_missing.items():
        if s == "MINOR":
            print(f"DEGRADE:GATE:MINOR:{f}:proceeding", file=sys.stderr)
